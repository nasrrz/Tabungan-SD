...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1 font-medium">Atur ruangan kelas dan wali kelas </p>
        </div>
        <a href="/admin/kelas/tambah" class="bg-sky-500 hover:bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-md shadow-sky-500/10 transition flex items-center gap-2 cursor-pointer w-fit">
            ...
            <table class="w-full text-left border-collapse min-w-[500px]">
                ...
                <tbody class="text-sm text-slate-700 divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/60 transition-all">
                        ...
                        <td class="p-4 font-black text-blue-950 text-base">{{ $kelas->nama_kelas }}</td>
                    <td class="p-4 font-medium text-slate-600">
                        @if($kelas->nama_guru)
                            @if(($kelas->jenis_kelamin ?? 'L') == 'L')
                                <span class="inline-flex items-center gap-1.5 text-slate-700 font-semibold bg-blue-50/60 text-blue-700 px-2.5 py-1 rounded-xl text-xs border border-blue-100">
                                    <i class="bi bi-person-circle"></i> Ustadz {{ $kelas->nama_guru }}
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1.5 text-slate-700 font-semibold bg-rose-50/60 text-rose-700 px-2.5 py-1 rounded-xl text-xs border border-rose-100">
                                    <i class="bi bi-person-circle"></i> Ustadzah {{ $kelas->nama_guru }}
                                </span>
                            @endif
                        @else
                            <span class="inline-flex items-center gap-1.5 text-rose-600 font-bold bg-rose-50 px-2.5 py-1 rounded-xl text-xs">
                        ...
                        <td class="p-4">
                            <div class="flex justify-center gap-2">
                                <button class="text-amber-600 bg-amber-50 hover:bg-amber-500 hover:text-white p-2 rounded-xl transition shadow-sm"><i class="bi bi-pencil-square"></i></button>
                                
                                <form action="/admin/kelas/hapus/{{ $kelas->id }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kelas ini? Semua data relasi plotting akan dikosongkan.')">
                                    @csrf
                                    @method('DELETE')
