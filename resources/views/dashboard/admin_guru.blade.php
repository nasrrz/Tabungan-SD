...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Kelola akun Ustadz/Ustadzah wali kelas</p>
        </div>
        <a href="/admin/guru/tambah" class="inline-flex items-center gap-2 bg-sky-500 hover:bg-sky-600 text-white px-5 py-2.5 rounded-lg text-sm font-semibold shadow-sm transition-colors">
            ...
            <table class="w-full text-left border-collapse min-w-[500px]">
                ...
                <tbody class="text-sm text-slate-700 divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="p-4 font-semibold text-slate-800">{{ $guru->nama }}</td>
                        <td class="p-4 font-mono text-slate-600">{{ $guru->username }}</td>
                        <td class="p-4 text-center">
                            @if($guru->jenis_kelamin == 'L')
                                <span class="inline-block px-2.5 py-1 bg-sky-50 text-sky-700 text-xs font-semibold rounded-md">Ustadz</span>
                            @else
                                <span class="inline-block px-2.5 py-1 bg-rose-50 text-rose-700 text-xs font-semibold rounded-md">Ustadzah</span>
                            @endif
                        </td>
                        <td class="p-4">
                            <div class="flex justify-center gap-2">
                                <a href="/admin/guru/edit/{{ $guru->id }}" class="text-amber-600 bg-amber-50 hover:bg-amber-500 hover:text-white p-2 rounded-lg transition-colors shadow-sm" title="Edit">
